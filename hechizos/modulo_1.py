...
def _raiz_progreso(idx_actual: int, total: int):
    ...
    mid = x_ini + ancho // 2
    lineas.append(
        f'<path d="M {x_ini} {yt} Q {x_ini+ancho//4} {yt-2} {mid} {yt} ' +
        f'Q {x_ini+3*ancho//4} {yt+2} {x_fin} {yt}" ' +
        'stroke="#2a2a3a" stroke-width="2" fill="none" stroke-linecap="round"/>' 
    )
    if x_actual > x_ini + 4:
        xm = (x_ini + x_actual) // 2
        lineas.append(
            f'<path d="M {x_ini} {yt} Q {xm} {yt-2} {x_actual} {yt}" ' +
    ...
    svg = (
        '<svg width="100%" viewBox="0 0 660 90" xmlns="http://www.w3.org/2000/svg" ' +
        'style="margin:0.2rem 0 0.5rem;max-height:80px">' +
        "".join(lineas) + '</svg>'
    )
